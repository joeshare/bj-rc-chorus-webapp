...
function validateTaskName(thiz){
    var id=_.uuid(50);
    thiz.view.validateTaskName({taskName:id},function(res){
        if(res.code!=0){
            validateTaskName(thiz)
        }
    })
    return id
}
function model(mv){
        ...
        listeners:{
            ...
            drop:function(e){
                if(_this.view.isCanvasActive){
                    ...
                    var id=validateTaskName(_this);
                    var objType='node';
                    var moduleData=_this.view.currentDragData;
                    var nodeName=moduleData.moduleAliasName;
                    ...
                    var container=canvasUtils.getContainer();
                    if(!container){ return;}
                    _this.view.addData2Points(opt);
                    container&&container.addChild(createNode(opt,canvasUtils.getImgObj()))
                    ...
                    if(moduleType=='fork'||moduleType=='join'){
                        ...
                        fetchAgent.fetchModuleProperty({moduleType,moduleName},function(res){
                            ...
                            if(res&&res.code*1==0){
                                ...
                                _this.view.messageAlert("获取组件属性失败");
                            }
                            _this.view.showModuleProperty({
                                code:0,
                                data:propertyData
                            ...
                            _this.view.setFetchModulePropertySourceData();
                        })
                    }
    ...
    this.dragDisable=function(type){
        if(type){
           $(".dom-dragable").addClass("dom-dragable")
        }else{
          $(".dom-dragable").removeClass("dom-dragable")
        }
    }
     this.setView=(v)=>{
         _this.view=v;
